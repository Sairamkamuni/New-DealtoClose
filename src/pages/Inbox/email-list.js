...
import { inboxmails } from 'common/data/mails'

const MailsList = ({ showToolbar = true }) => {
  const dispatch = useDispatch()

  const { mails } = []

  const [mailList, setmailList] = useState(inboxmails)
      ...
      {showToolbar && (
        <div className="btn-toolbar p-3" role="toolbar">
          <div className="btn-group me-2 mb-2 mb-sm-0">
            <Button type="button" color="primary">  <i className="fa fa-inbox" /> </Button>
            <Button type="button" color="primary"> <i className="fa fa-exclamation-circle" /> </Button>
            <Button type="button" color="primary" onClick={handleClick}> <i className="far fa-trash-alt" /> </Button>
          </div>
          ...
          <Dropdown isOpen={more_Menu} toggle={() => { setmore_Menu(!more_Menu) }} className="btn-group me-2 mb-2 mb-sm-0" >
            <DropdownToggle className="btn btn-primary  dropdown-toggle" tag="div" > More <i className="mdi mdi-dots-vertical ms-2" /> </DropdownToggle>
            <DropdownMenu className="dropdown-menu-end">
        ...
        {map(mailList, (mail, index) => (
          <li key={index}>
            <div className="col-mail col-mail-1">
              ...
              <Link to="#" className="title"> {mail.name} </Link>
